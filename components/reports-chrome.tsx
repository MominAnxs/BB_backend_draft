...
/**
 * Shared Reports chrome — filter bar + AI Insights drawer.
 *
 * Extracted from app/adminland/reports/layout.tsx so the exact same UI
 * can be rendered both at /adminland/reports AND inside the Super Admin
 * Home's Reports tab.
 *
 * The Adminland route still uses the Next.js layout; this file contains
 * the reusable component that both contexts render.
 */

import { useState, useEffect, useRef, useCallback } from 'react';
import {
  Calendar, ChevronDown, Sparkles, X, Send, ArrowRight, RotateCcw, Copy, Check,
  TrendingUp, Users, AlertCircle, Briefcase, Target, Zap, ChevronRight,
} from 'lucide-react';

// ═══════════════════════════════════════════════════════════════════
// PUBLIC CONSTANTS — tab list + HR sections (exported so layout can reuse)
// ═══════════════════════════════════════════════════════════════════
...
export type ReportsDateRange = 'ytd' | 'mtd' | 'weekly' | 'q1' | 'q2' | 'q3' | 'q4';
export type ReportsDepartment = 'All' | 'Finance' | 'Performance Marketing';
...
// ═══════════════════════════════════════════════════════════════════
// AI INSIGHT DRAWER COMPONENT (exported)
// ═══════════════════════════════════════════════════════════════════

export function AIInsightDrawer({
  isOpen, onClose, activeTab, hrSection,
}: {
              ...
              {response.keyMetric && (
                ...
                >
                  ...
                  <div className="flex items-end gap-3 mt-1.5">
                    <span className={`text-display-sm ${response.keyMetric.good ? 'text-emerald-600' : 'text-rose-500'}`}>
...
// ═══════════════════════════════════════════════════════════════════
// REPORTS FILTER BAR + CONTENT WRAPPER (exported)
// ═══════════════════════════════════════════════════════════════════

/**
 * Exact replica of the filter bar at /adminland/reports.
 *
 * - `activeTab`: currently-selected report view. Defaults to 'overview'.
 * - `hrSection`: HR Reports sub-section when activeTab === 'hr-reports'.
 * - `onTabChange` / `onHRSectionChange`: callbacks for dropdown changes.
 *   If omitted, the dropdowns push to `/adminland/reports/*` via window.location.
 * - `children`: the report body (e.g. <Overview /> for the overview tab).
 * - `globalDateRange` / `globalDepartment`: controlled state, reported back via
 *   onDateRangeChange / onDepartmentChange so callers can pass them to Overview.
 *
 * Layout note: matches the adminland layout exactly, including the `-mx-6 -mt-6`
 * negative margin on the filter bar that bleeds it to the container edges.
 */
interface ReportsChromeProps {
  ...
  onHRSectionChange?: (sectionKey: string) => void;
  globalDateRange: ReportsDateRange;
  onDateRangeChange: (v: ReportsDateRange) => void;
  globalDepartment: ReportsDepartment;
  onDepartmentChange: (v: ReportsDepartment) => void;
  /** Hide the Department filter dropdown. Defaults to false. */
  hideDepartmentFilter?: boolean;
  /** Hide the Report-view (Overview/Growth/...) switcher dropdown. Defaults to false. */
...
export function ReportsChrome({
  ...
  const handleTabChange = (tabId: string) => {
    if (onTabChange) onTabChange(tabId);
      ...
      if (tab && typeof window !== 'undefined') window.location.href = tab.href;
    }
  };

  const handleHRSectionChange = (sectionKey: string) => {
    if (onHRSectionChange) onHRSectionChange(sectionKey);
    else if (typeof window !== 'undefined') {
      window.location.href = `/adminland/reports/hr-reports?section=${sectionKey}`;
    }
  };
              ...
              <>
                <div className="relative">
                  ...
                  <select
                    ...
                    onChange={(e) => onDateRangeChange(e.target.value as ReportsDateRange)}
                ...
                {!hideDepartmentFilter && (
                  <div className="relative">
                    ...
                    <select
                      ...
                      onChange={(e) => onDepartmentChange(e.target.value as ReportsDepartment)}
            ...
            {!hideViewSwitcher && (
              <div className="relative">
                ...
                <select
                  ...
                  className="appearance-none bg-[#EEF1FB] text-[#204CC7] pl-3 pr-8 py-1.5 rounded-md text-caption font-semibold border border-[#204CC7]/20 hover:bg-[#E4E8F9] focus:outline-none focus:ring-2 focus:ring-[#204CC7]/30 transition-all cursor-pointer"
                >
                  {reportTabs.map((tab) => (
